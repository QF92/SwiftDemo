...
fileprivate func convertAmrToWave(isWB : Bool, data : Data) -> Data? {
    ...
    var amrData = data.subdata(in: magicNumberCount..<data.count)
    ...
    repeat {
        if amrData.count >= Int(frameSize) {
            let frameData = amrData.subdata(in: (0..<Int(frameSize)))
            ...
            amrData.removeSubrange((0..<Int(frameSize))
)
        }
        else {
            break
...
fileprivate extension Data {
    ...
    func readChunkFmt(offset : Int) -> ChunkFmt? {
        ...
        return readData(range:(offset..<offset + 24), type: ChunkFmt.self)
    }
    ...
    func pcmFrame(numChannels : UInt16, bitsPerSample : UInt16, frameSize : Int, offset : Int) -> [Int16]? {
        ...
        if numChannels == 1, bitsPerSample == 8 {
            let bytes = readBufferData(range:(offset..<(offset+frameSize)), type: UInt8.self, capacity: frameSize)
            ...
            let bytes = readBufferData(range:(offset..<(offset+frameSize)), type: Int16.self, capacity: frameSize/2)
