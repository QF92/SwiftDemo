...
/// 自定义导航栏
    open lazy var id_customNavBar: CustomNavgationView = {
    ...
    open lazy var id_rightBtn:UIButton = {
       let btn = UIButton()
        ...
        btn.addTarget(self, action: #selector(BaseViewController.rightBtnClick), for: UIControl.Event.touchUpOutside)
        return btn
    }()
    /// 右边第二个按钮
