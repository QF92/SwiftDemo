...
class ImagePickerVC: BaseViewController {
    ...
    let picker2 = ImagePicker()
    let picker3 = ImagePicker()
    let picker4 = ImagePicker()
    let picker5 = ImagePicker()
        
    lazy var photoView: PhotoView = {
        let photo = PhotoView.init(frame: CGRect.init(x: 0, y: 200, width: KScreenWidth, height: 80))
        photo.backgroundColor = UIColor.systemGroupedBackground
        return photo
    }()
   ...
   override func viewDidLoad() {
       ...
       for i in 0..<titleArr.count {
        ...
        // 先拼接一个图片
        self.photoView.picArr.append(UIImage(named: "takePictures")!)
        // 点击了删除，更新界面
    ...
    private func setupPhoto1() {
        ...
        ImagePicker.convertAssetArrToOriginImage(assetArr: assetArr, scale: 0.1, successClouse: {[weak self] (image,assetItem) in
...
@objc func clickBtn(btn: UIButton) {
    if btn.tag == 100 {
            ...
            picker.id_setupImagePickerWith(maxImagesCount: 10) {
                (assrtArr, img) in
            }
        }
        if btn.tag == 101 {
        ...
        if btn.tag == 103 {
            ...
            picker4.id_setupImagePickerWith(maxImagesCount: 10) {
                (assrtArr, img) in
            }
        }
        if btn.tag == 104 {
            picker5.tineColor = UIColor.green
            picker5.id_setupImagePickerWith(maxImagesCount: 10) {
                (assrtArr, img) in
        }
    }
  }
