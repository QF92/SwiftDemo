...
class EditorViewController: UIViewController {
   ...
   // 底部控件
    ...
    @objc func showPencilView(){
        ...
        self.choosePencilView.id_y = self.bottomView.id_y
        UIView.animate(withDuration: 0.3, animations: { 
            self.choosePencilView.id_y = self.bottomView.id_y-40
        }) { (comp) in
        }
    }
    //MARK: - 选择画笔结束
    @objc func choosePencilViewDismiss() {
        UIView.animate(withDuration: 0.3, animations: {
            self.choosePencilView.id_y = KScreenHeight
        }) { (true) in
...
extension EditorViewController {
    fileprivate func initView() {
        ...
        if scaleImage.size.height < KScreenHeight-44-50 {
            drawBoardImageView.id_y = (KScreenHeight-44-50-scaleImage.size.height)*0.5
        }
