...
public enum ImagePickerStatusBarType {
    case black   // 黑色
    case white   // 白色
}

public typealias ImagePickerClouse = (Array<PHAsset>,UIImage?)->()
...
class ImagePicker: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    public var imagePickerClouse: ImagePickerClouse?
        ...
        public var singleImageChooseType: ImagePickerType?
        // 设置相机在外部，默认不在外部
        ...
        // 配置状态栏的颜色
        public var statusBarType: ImagePickerStatusBarType = .white
        // 相册中是否展示拍照图片
        public var showCamaroInPicture = false
        // 文字和图片的颜色 85 182 55
        public var tineColor = DealistConfig.share.mainColor {
            didSet {
...
@objc public func id_setupImagePickerWith(maxImagesCount: Int,superVC:UIViewController? = nil,didChooseImageSuccess:@escaping (Array<PHAsset>,UIImage?)->()) {
            ...
            self.imagePickerClouse = didChooseImageSuccess
        ...
        func gotoPhoto(MaxImagesCount: Int) {
            ...
            CLPickersTools.instence.authorize(authorizeClouse: { (state) in
                if state == .authorized {
                    let photo = ImagePickersNavigationVC.share.initWith(
...
@objc public static func convertAssetArrToOriginImage(assetArr:Array<PHAsset>,scale:CGFloat,successClouse:@escaping (UIImage,PHAsset)->(),failedClouse:@escaping ()->()) {
            ...
            for item in assetArr {
                ...
                ImagePicker.getAssetOrigin(asset: item, dealImageSuccess: { (img, info) in
                    if img != nil {
                        // 对图片压缩
    ...
    let info = convertFromUIImagePickerControllerInfoKeyDictionary(info)
            ...
            if type == "public.image" {
                CLPickersTools.instence.authorizeSave { (state) in
                    if state == .authorized {
                        ...
                        UIImageWriteToSavedPhotosAlbum(photo as! UIImage, self, #selector(ImagePicker.image(_:didFinishSavingWithError:contextInfo:)), nil)
                    } else {
                        DispatchQueue.main.async(execute: {
                ...
                CLPickersTools.instence.getAssetOrigin(asset: phasset) { (img, info) in
                    ...
                    if img != nil {
                        ...
                        if self.singleImageChooseType == .singlePictureCrop && self.singleModelImageCanEditor != true {  // 单选
                            ...
                            if self.imagePickerClouse != nil {
                                self.imagePickerClouse!([phasset],image)
                            }
                        }
        ...
        func crop(image: UIImage,phasset: PHAsset) {
            ...
            cutVC.clCropClouse = {[weak self] (cutImg) in
                if self?.imagePickerClouse != nil {
                    self?.imagePickerClouse!([phasset],cutImg)
                }
        ...
        func editor(image: UIImage,phasset: PHAsset) {
            ...
            editorVC.editorImageComplete = {[weak self](img) in
                ...
                if self?.imagePickerClouse != nil {
                    self?.imagePickerClouse!([phasset],img)
                }
            }
