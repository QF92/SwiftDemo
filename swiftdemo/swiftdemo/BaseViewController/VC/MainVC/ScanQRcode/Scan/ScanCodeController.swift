...
class ScanCodeController: BMScanDefaultCotroller {

     open var successClouse: IDScanViewControllerSuccessClouse?
     open var failClouse: IDScanViewControllerFailClouse?

        open lazy var customNavBar: ScanNavgationView = {
            let nav = ScanNavgationView.init(frame: CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: KNavgationBarHeight))
            return nav
        }()
        // 返回按钮
        ...
        override open func feetColor() -> UIColor! {
            return DealistConfig.share.mainColor
        }
    ...
    extension ScanCodeController: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
        //选取照片后的回调方法
