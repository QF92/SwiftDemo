...
public enum ButtonGradientDirection {
...
class BaseButton: UIButton {
   ...
   /// 图片位置
    open var id_imagePosition = ButtonPosition.left {
        didSet {
            self.layoutIfNeeded()
        }
    }
    ...
    /// 按钮类型
    open var id_type: ButtonTypes = .normal {
        didSet {
            self.common()
        }
    }
    ...
    private var loadingView: RefreshLoadingView = {
        let loading = RefreshLoadingView.init(frame: CGRect.zero)
        loading.radius = 7.5
        return loading
    }()
    ...
    //MARK: - 渐变色
    public func id_setupGradient(gradientColors: [UIColor], gradientDirection direction: ButtonGradientDirection, gradientFrame: CGRect? = nil) {
    ...
    func setupPrimary() {
        self.backgroundColor = DealistConfig.share.mainColor
        self.layer.borderWidth = 0
        self.setTitleColor(UIColor.white, for: UIControl.State.normal)
    }
    func setupError() {
        self.backgroundColor = RGBAColorAlpha(233, 80, 79, 1)
        self.layer.borderWidth = 0
        self.setTitleColor(UIColor.white, for: UIControl.State.normal)
    }
    func setupNormal() {
