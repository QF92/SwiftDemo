...
import UIKit
public enum DialogUtilType {
    case normal  // 类似系统的弹框
    case image  // 带有图片的弹框
    case input // 可输入文字的弹框
    case custom // 自定义内容
}
public enum DialogUtilImageType {
    case success
    case fail
    case warning
}

class DialogUtil: Operation {
    lazy var coverView: UIView = {
        ...
        var normalView: DialogNormalView? = DialogNormalView()   // 默认
        var imgView: DialogImageView? = DialogImageView()
    ...
    var superComponent: UIView = (UIApplication.shared.windows.first ?? UIView())
        ...
        init(
            ...
            type: DialogUtilType? = nil) {
            
            super.init()
            
            self.imageType = success
            self.type = type ?? DialogUtilType.normal
        ...
        init(
            ...
            self.type = type ?? DialogUtilType.input
        ...
        init(
            ...
            self.type = type ?? DialogUtilType.custom
        ...
        func setupNormalView(title: String? = nil,
                             ...
                             rightHandler:(()->())? = nil,
                             countDownNumber: Int? = nil) {
            self.normalView = DialogNormalView()
        ...
        func setupInputView(msg: String? = nil,
                            ...
                            leftHandler: ((String)->())? = nil,
                            rightHandler:((String)->())? = nil) {
            self.inputView = DialogInputView()
        ...
        func setupCustomView(msg: String? = nil,
                             ...
                             leftHandler: ((UIView?)->())? = nil,
                             rightHandler:((UIView?)->())? = nil) {
            self.customView = DialogCustomView()
        ...
        func showNormalView() {
            self.superComponent.addSubview(self.normalView ?? DialogNormalView())
            self.normalView?.msgLabel.textAlignment = DialogManager.share.textAlignment
            self.normalView?.setNeedsLayout()
            
            if DialogManager.share.supportAnimate {
                self.normalView?.layer.add(DialogManager.share.animate, forKey: nil)
            }
        }
        func showImageView() {
            self.superComponent.addSubview(self.imgView ?? DialogImageView())
            self.imgView?.msgLabel.textAlignment = DialogManager.share.textAlignment
            
            if self.imageType == .success {
                self.imgView?.iconView.image = DialogManager.share.successImage
            } else if self.imageType == .fail {
                self.imgView?.iconView.image = DialogManager.share.failImage
            } else if self.imageType == .warning {
                self.imgView?.iconView.image = DialogManager.share.warnImage
            } else {
            ...
            if DialogManager.share.supportAnimate {
                self.imgView?.layer.add(DialogManager.share.animate, forKey: nil)
            }
        }
        func showInputView() {
            self.superComponent.addSubview(self.inputView ?? DialogInputView())
            self.inputView?.msgLabel.textAlignment = DialogManager.share.textAlignment
            self.inputView?.setNeedsLayout()
            
            if DialogManager.share.supportAnimate {
                self.inputView?.layer.add(DialogManager.share.animate, forKey: nil)
            }
        }
        func showCustomView() {
            self.superComponent.addSubview(self.customView ?? DialogCustomView())
            self.customView?.msgLabel.textAlignment = DialogManager.share.textAlignment
            self.customView?.setNeedsLayout()
            
            if DialogManager.share.supportAnimate {
                self.customView?.layer.add(DialogManager.share.animate, forKey: nil)
            }
        }
    }

    extension DialogUtil {
        ...
        func finish() {
            ...
            if DialogManager.share.supportQuene == false {
                DialogManager.share.cancelAll()
            }
        }
    }
